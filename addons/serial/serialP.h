...
#define chip_atomic_bgn(flags)  do { flags = rt_spin_lock_irqsave(&splock); } while (0)
#define chip_atomic_end(flags)  do { rt_spin_unlock_irqrestore(flags, &splock); } while (0)
...
#else
#define chip_atomic_bgn(flags)  do { hard_save_flags_and_cli(flags); } while (0)
#define chip_atomic_end(flags)  do { hard_restore_flags(flags); } while (0)
#define buf_atomic_bgn(flags, mbx)  do { hard_save_flags_and_cli(flags); } while (0)
