...
do { \
	...
	cpuid = rtai_cpuid(); \
	lflags = xchg(&adp_root->cpudata[cpuid].status, 1); \
...
//1)	lflags = adeos_test_and_stall_pipeline_from (adp_root);
	...
	if (rtai_realtime_irq[irq].handler) {
		int retval;
		RTAI_SCHED_ISR_LOCK();
		retval = ((int (*)(int, void *))rtai_realtime_irq[irq].handler)(irq, rtai_realtime_irq[irq].cookie);
                if (rtai_realtime_irq[irq].retmode && retval) {
			rtai_cli();
			adp_root->cpudata[cpuid].status = lflags;
//1)			adeos_restore_pipeline_nosync(adp_root, lflags, cpuid);
...
//1)	if (!adeos_test_pipeline_from(adp_root)) {
		rtai_sti();
...
asmlinkage int rtai_syscall_entry (long bx, unsigned long cx_args, long long *dx_retval, long si, long di, long bp, unsigned long ax_srq)
{
