...
{
	unsigned long cpuid, irq = regs->orig_rax & 0xFF;
...
static inline long long rtai_usrq_dispatcher (unsigned srq, unsigned long label)
{
...
int __rtai_hal_init (void)
{
	...
	flags = rtai_critical_enter(NULL);
	adeos_default_irq_handler = adeos_set_irq_handler(rtai_hirq_dispatcher);
