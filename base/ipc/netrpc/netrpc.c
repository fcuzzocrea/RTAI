...
static void net_resume_task(int sock, struct portslot_t *p)
{
	...
	if (all_ok) {
		...
		if (my->is_hard) {
			while (hard_rt_recvfrom(p->socket[1], msg, MAX_MSG_SIZE, 0, addr, (socklen_t *)&i) == -EAGAIN);
		} else {
...
recvryh:
	...
	while (rt_sem_wait(sem) < RTE_LOWERR) {
		wsize = hard_rt_recvfrom(sock, msg, MAX_MSG_SIZE, 0, addr, (socklen_t *)&w2size);
		if (decode) {
			decode(portslotp, msg, wsize, RPC_SRV);
		}
		if (portslotp->owner != par->owner)	{
...
RTAI_SYSCALL_MODE long long _rt_net_rpc(long fun_ext_timed, long type, void *args, int argsize, int space, unsigned long partypes)
{
	...
	if ((port = PORT(fun_ext_timed)) > 0) {
		...
		if ((portslotp = portslot + port)->task < 0) {
			...
			if ((rsize = portslotp->hard ? hard_rt_recvfrom(portslotp->socket[1], msg, MAX_MSG_SIZE, 0, &addr, (socklen_t *)&i) : soft_rt_recvfrom(portslotp->socket[0], msg, MAX_MSG_SIZE, 0, &addr, &i))) {
				if (decode) {
...
//					mbx_send_if(portslotp->mbx, msg, rsize);
	...
	if (port > 0) {
		...
		rsize = portslotp->hard ? hard_rt_recvfrom(portslotp->socket[1], msg, MAX_MSG_SIZE, 0, &addr, (socklen_t *)&port) : soft_rt_recvfrom(portslotp->socket[0], msg, MAX_MSG_SIZE, 0, &addr, &port);
		if (decode) {
