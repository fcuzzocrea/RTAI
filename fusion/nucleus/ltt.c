...
struct xnltt_evmap xnltt_evtable[] = {
    ...
    [rtai_ev_switch] = { "RTAI switch", "%s -> %s", -1 },
    [rtai_ev_fault] = { "RTAI fault", "thread=%s, location=%p, trap=%d", -1 },
    [rtai_ev_callout] = { "RTAI callout", "type=%s, thread=%s", -1 },
    [rtai_ev_finalize] = { "RTAI finalize", "%s -> %s", -1 },
    [rtai_ev_thrinit] = { "RTAI thread init", "thread=%s", -1 },
    ...
    [rtai_ev_cpumigrate] = { "RTAI CPU migrate", "thread=%s, cpu=%d", -1 },
    [rtai_ev_sigdispatch] = { "RTAI signal dispatch", "thread=%s, sigpend=0x%x", -1 },
    [rtai_ev_thrboot] = { "RTAI thread bootstrap", "thread=%s", -1 },
    [rtai_ev_tmtick] = { "RTAI timer tick", "runthread=%s", -1 },
    [rtai_ev_sleepon] = { "RTAI sleepon", "thread=%s, sync=%p", -1 },
    [rtai_ev_wakeup1] = { "RTAI wakeup1", "thread=%s, sync=%p", -1 },
    [rtai_ev_wakeupx] = { "RTAI wakeupx", "thread=%s, sync=%p", -1 },
    [rtai_ev_syncflush] = { "RTAI syncflush", "sync=%p, reason=0x%x", -1 },
    [rtai_ev_syncforget] = { "RTAI syncforget", "thread=%s, sync=%p", -1 },
    [rtai_ev_lohandler] = { "RTAI lohandler", "type=%d, task=%s, pid=%d", -1 },
    [rtai_ev_primarysw] = { "RTAI primary switch", "thread=%s", -1 },
    [rtai_ev_primary] = { "RTAI primary exec", "thread=%s", -1 },
    [rtai_ev_secondarysw] = { "RTAI secondary switch", "thread=%s", -1 },
    [rtai_ev_secondary] = { "RTAI secondary exec", "thread=%s", -1 },
    [rtai_ev_shadowmap] = { "RTAI shadow mapping", "thread=%s, pid=%d, prio=%d", -1 },
    [rtai_ev_shadowunmap] = { "RTAI shadow unmapping", "thread=%s, pid=%d", -1 },
