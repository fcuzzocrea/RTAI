...

{
    ...
    sem = compat_h2obj_validate(sem,COMPAT_SEM_MAGIC,SEM);
...

{
    ...
    if (sem->type == RES_SEM)
	{
	if (compat_current_task() != sem->owner)
	    {
	...
	sem->owner = thread2rtask(xnsynch_wakeup_one_sleeper(&sem->synch_base));
