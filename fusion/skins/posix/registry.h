...
#define pse51_ref_p(node) ((node)->refcount)
...
/* Get an existing node: oflags are _open style flags. If error is 0 and *nodep
   is NULL, then a new node should be added with node_add*. */
int pse51_node_get(pse51_node_t **nodep,
                   const char *name,
                   unsigned long magic,
                   long oflags);

/* bind a node : use node_add for simple objects, node_add_start and
   node_add_finished for objects which need to be initialized outside of any
   critical sections. */
