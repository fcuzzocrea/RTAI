...
/* including linux/module.h in user space does not work with SMP enabled kernel
   we hence paste here the two definitions needed for rt_lxrt_loaded.
*/
...
/**
 * Check if LXRT is loaded in the Linux kernel.
 * 
 * @retval 0 on success,
 * @retval -1 on failure.
 */
RTAI_PROTO(int, rt_lxrt_loaded,(void))
{
    ...
    return query_module("rtai_lxrt", QM_INFO, &modInfo, 
			sizeof(modInfo), &retSize);
}
