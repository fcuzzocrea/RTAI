...
#define rt_heap_open(name, size, suprt)  \
	_rt_shm_alloc(0, name, size + sizeof(struct spinlock *) + 1, suprt, 1)
...
#define rt_heap_open_adr(start, name, size, suprt)  \
	_rt_shm_alloc(start, name, size + sizeof(struct spinlock *) + 1, suprt, 1)
 ...
 *
 * @param start_address size is the amount of required shared memory.
 * 
 * @param name is an unsigned long identifier;
 * 
 * @param size is the amount of required shared memory;
