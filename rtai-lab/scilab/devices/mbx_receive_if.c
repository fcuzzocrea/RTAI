...
void inp_mbx_receive_if_init(int port,int nch,char * sName,char * IP)
{
  long Target_Node;
  long Target_Port=0;
  MBX * mbx;
  struct sockaddr_in addr;
    ...
    strcpy(inpDevStr[id].IOName,"mbx_receive_if inp");
...
void inp_mbx_receive_if_input(int port, double * y, double t)
{
  ...
  if(!RT_mbx_receive_if(inpDevStr[id].l1, inpDevStr[id].l2, mbx ,&data, sizeof(data))) {
    for(i=0;i<ntraces;i++) {
      old_val[i] = data.u[i];
    }
  }
