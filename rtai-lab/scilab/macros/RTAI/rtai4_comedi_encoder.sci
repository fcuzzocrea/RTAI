function [x,y,typ] = rtai4_comedi_encoder(job,arg1,arg2)
  ...
  case 'set' then
    ...
    while %t do
      [ok, number, name, chA, chB, chZ, initval, index, cmode, exprs]=..
      getvalue(['Set RTAI-COMEDI Encoder block parameters';
                'In up/down mode, set B to -1 to input the';
                'up/down signal on the P0.6 or P0.7 pin'],..
      ['Counter No';
      'Device';
      'A PFI';
      'B PFI';
      ...
      list('vec',1,'str',1,'vec',1,'vec',1,'vec',1,'vec',1,'vec',1,'vec',1),exprs)
