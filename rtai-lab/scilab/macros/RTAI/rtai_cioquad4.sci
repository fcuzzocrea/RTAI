function [x,y,typ]=rtai_cioquad4(job,arg1,arg2)
//
...
case 'set' then
  ...
  oldlb=label(1)
  while %t do
    [ok,port,cioquad4_addr,modul,res,prec,rot,rest,lab]=..
        getvalue('Set CIOQUAD4 data block parameters',..
        ['Port';
        'CARD Addr';
	'Modul';
	'Resolution';
	'Precision [1,2,4]'
	'Rotation [+1,-1]'
	'Reset [initial 0, Continous 1]'],..
         list('vec',1,'str',1,'vec',1,'vec',1,'vec',1','vec',1,'vec',1),label(1))

    if ~ok then break,end
    label(1)=lab
    funam='i_cioquad4_' + string(port);
    xx=[];ng=[];z=0;
    nx=0;nz=0;
    i=[];
    o=1;nout=1;
...
case 'define' then
  port=1; res=1000; prec=4;rot=1;rest=1;modul=1;
  rt_par=[res,prec,rot,rest,modul]
  rpar=rt_par(:)
  cioquad4_addr='0x300'
  ...
  label=list([sci2exp(port),cioquad4_addr,sci2exp(modul),sci2exp(res),sci2exp(prec),sci2exp(rot),sci2exp(rest)],[])

  gr_i=['xstringb(orig(1),orig(2),''CIOQUAD4'',sz(1),sz(2),''fill'');']
  ...
  textmp=[
	 ...
	 l3 = string(rot) + ',' + string(rest) +',0';
  	 ...
  	 ttext=[ttext;'int '+funam+"_bloc_outputs(scicos_block *block,int flag)";
         ...
         '#ifdef MODEL'
         "  inp_cioquad4_input(" + string(port) + ",y,t);";
         '#endif'
