...
function [ok,tt]=getCode_comedi_datain(funam)
  ...
  textmp($+1)='  case 4:'
  textmp($+1)='    blk=inp_rtai_comedi_data_init('+string(ch)+',""'+name + '"",'+string(com_range)+','+string(aref)+');'
  textmp($+1)='    break;'; 
  textmp($+1)='  case 1:'
  textmp($+1)='    inp_rtai_comedi_data_input(blk,y,t);'
  textmp($+1)='    block->outptr[0][0]=y[0];'
  textmp($+1)='    break;'
  textmp($+1)='  case 5:'
  textmp($+1)='    inp_rtai_comedi_data_end(blk);'
  textmp($+1)='    break;'
  textmp($+1)='  }'
  textmp($+1)='#endif'
  textmp($+1)='}'
