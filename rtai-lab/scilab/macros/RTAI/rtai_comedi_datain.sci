function [x,y,typ]=rtai_comedi_datain(job,arg1,arg2)
//
...
case 'set' then
  ...
  oldlb=label(1)
  while %t do
    [ok,port,ch,name,com_range,aref,lab]=..
	...
	'Range';
        'Aref'],..
         list('vec',1,'vec',1,'str',1,'vec',1,'vec',1'),label(1))
...
case 'define' then
  port=1;ch=0;com_range=0;aref=0; 
  rt_par=[com_range,aref,0,0,0]
  rpar=rt_par(:)
  name='comedi0'
  ...
  label=list([sci2exp(port),sci2exp(ch),name,sci2exp(com_range),sci2exp(aref)],[])

  gr_i=['xstringb(orig(1),orig(2),''COMEDI DATAIN'',sz(1),sz(2),''fill'');']
...
if tt==[] then
  ...
  textmp=[
	 ...
	 'void '+funam+'(scicos_block *block,int flag)';
  	 ...
  	 textmp($+1)='   break;';
    	 l1 = '  inp_rtai_comedi_data_init(' + string(port) + ',' + string(ch) + ',';
    	 l2 = '""' + name + '"","""",' + string(com_range) + ',' + string(aref) + ',0,0,0';
  	 ...
  	 ttext=[ttext;'int '+funam+"_bloc_outputs(scicos_block *block,int flag)";
         ...
         '#ifdef MODEL'
         "  inp_rtai_comedi_data_input(" + string(port) + ",y,t);";
         '#endif'
 	 ...
 	 textmp($+1)='  case 5: '
         ...
         "{";
         '#ifdef MODEL'
         "  inp_rtai_comedi_data_end(" + string(port) + ");";
         '#endif'
         "  return 0;";
         "}"];
  	 textmp($+1)='  }'
