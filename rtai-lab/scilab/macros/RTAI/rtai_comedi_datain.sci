...
case 'set' then
  ...
  while %t do
    ...
    label(1)=lab
    funam='i_comedi_data_' + name + '_' + string(ch);
    xx=[];ng=[];z=0;
    nx=0;nz=0;
    i=[];
    o=1;nout=1;
    ...
    [ok,tt]=getCode_comedi_datain(funam)
    if ~ok then break,end
...
function [ok,tt]=getCode_comedi_datain(funam)
   textmp=[
          '#ifndef MODEL'
          '#include <math.h>';
          '#include <stdlib.h>';
  ...
  textmp($+1)='  case 4:'
  textmp($+1)='    port=inp_rtai_comedi_data_init('+string(ch)+',""'+name + '"",'+string(com_range)+','+string(aref)+');'
  textmp($+1)='    break;'; 
  textmp($+1)='  case 1:'
  textmp($+1)='    inp_rtai_comedi_data_input(port,y,t);'
  textmp($+1)='    block->outptr[0][0]=y[0];'
  textmp($+1)='    break;'
  textmp($+1)='  case 5:'
  textmp($+1)='    inp_rtai_comedi_data_end(port);'
  textmp($+1)='    break;'
  textmp($+1)='  }'
  textmp($+1)='#endif'
  textmp($+1)='}'
