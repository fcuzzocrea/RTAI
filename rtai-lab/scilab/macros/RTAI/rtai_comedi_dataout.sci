...
function [ok,tt]=getCode_comedi_dataout(funam)
  ...
  textmp($+1)='  case 4:'
  textmp($+1)='    port=out_rtai_comedi_data_init('+ string(ch)+',""' + name + '"",'+string(com_range)+','+string(aref)+');'
  textmp($+1)='    break;'; 
  textmp($+1)='  case 2:'
  textmp($+1)='    u[0]=block->inptr[0][0];'
  textmp($+1)='    out_rtai_comedi_data_output(port,u,t);'
  textmp($+1)='    break;'
  textmp($+1)='  case 5:'
  textmp($+1)='    out_rtai_comedi_data_end(port);'
  textmp($+1)='    break;'
  textmp($+1)='  }'
  textmp($+1)='#endif'
  textmp($+1)='}'
