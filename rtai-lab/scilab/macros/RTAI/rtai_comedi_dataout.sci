...
case 'define' then
  ...
  gr_i=['xstringb(orig(1),orig(2),''COMEDI DATAOUT'',sz(1),sz(2),''fill'');']
...
if tt==[] then
  ...
  textmp($+1)='{'
  	 ...
  	 textmp($+1)='   break;';
    	 l1 = '  out_rtai_comedi_data_init(' + string(port) + ',' + string(ch) + ',';
  ...
  ttext=[ttext;'int '+funam+"_bloc_outputs(scicos_block *block,int flag)";
	  "{";
	   ...
	   "  u[0]=block->inptr[0][0];";
	   '#ifdef MODEL'
           "  out_rtai_comedi_data_output(" + string(port) + ",u,t);";
	   '#endif'
	   "  return 0;";
           "}"];
  ...
  textmp($+1)='  case 5: '
      ...
      textmp($+1)='   break;'; 
	   ...
	   "{";
	   '#ifdef MODEL'
	   "  out_rtai_comedi_data_end(" + string(port) + ");";
	   '#endif'
	   "  return 0;";
           "}"];
  textmp($+1)='  }'
