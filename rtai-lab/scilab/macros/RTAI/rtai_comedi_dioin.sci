...
case 'set' then
  ...
  while %t do
    ...
    label(1)=lab
    funam='i_comedi_dio_' + string(port);
    xx=[];ng=[];z=0;
    nx=0;nz=0;
    i=[];
    o=1;nout=1;
...
if tt==[] then
  ...
  textmp=[
	 ...
	 'void '+funam+'(scicos_block *block,int flag)';
  	 ...
  	 textmp($+1)='   break;';
    	 l1 = '  inp_rtai_comedi_dio_init(' + string(port) + ',' + string(ch) + ',';
    	 l2 = '""' + name + '"","""",0,0,0,0,0';
  	 ...
  	 ttext=[ttext;'int '+funam+"_bloc_outputs(scicos_block *block,int flag)";
         ...
         '#ifdef MODEL'
         "  inp_rtai_comedi_dio_input(" + string(port) + ",y,t);";
         '#endif'
 	 ...
 	 textmp($+1)='  case 5: '
         ...
         "{";
         '#ifdef MODEL'
         "  inp_rtai_comedi_dio_end(" + string(port) + ");";
         '#endif'
         "  return 0;";
         "}"];
  	 textmp($+1)='  }'
