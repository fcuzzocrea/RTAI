...
case 'set' then
  ...
  while %t do
    ...
    label(1)=lab
    funam='o_comedi_data_' + name + string(ch);
    xx=[];ng=[];z=0;
    nx=0;nz=0;
    o=[];
    i=1;nin=1;
    ...
    [ok,tt]=getCode_comedi_dioout(funam)
    if ~ok then break,end
...
function [ok,tt]=getCode_comedi_dioout(funam)
   textmp=[
          '#ifndef MODEL'
          '#include <math.h>';
          '#include <stdlib.h>';
  ...
  textmp($+1)='  case 4:'
  textmp($+1)='    port=out_rtai_comedi_dio_init('+ string(ch)+',""' + name + '"",'+string(thresh)+');'
  textmp($+1)='    break;'; 
  textmp($+1)='  case 2:'
  textmp($+1)='    u[0]=block->inptr[0][0];'
  textmp($+1)='    out_rtai_comedi_dio_output(port,u,t);'
  textmp($+1)='    break;'
  textmp($+1)='  case 5:'
  textmp($+1)='    out_rtai_comedi_dio_end(port);'
  textmp($+1)='    break;'
  textmp($+1)='  }'
  textmp($+1)='#endif'
  textmp($+1)='}'
