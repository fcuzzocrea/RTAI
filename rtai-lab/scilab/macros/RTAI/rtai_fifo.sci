...
case 'define' then
  ...
  label=list([sci2exp(port),sci2exp(in),sci2exp(fifon)],[])

  gr_i=['xstringb(orig(1),orig(2),''RTAI Fifo'',sz(1),sz(2),''fill'');']
...
if tt==[] then
  ...
  textmp($+1)='   break;'; 
    l1 = '  out_rtai_fifo_init(' + string(port) + ',' + string(nin) + ',';
    l2 = '""' + string(fifon) + '"","""",0,0,0,0,0';
  ...
  ttext=[ttext;'int '+funam+"_bloc_outputs(scicos_block *block,int flag)";
	  "{";
	   ...
	   '#ifdef MODEL'
           "  out_rtai_fifo_output(" + string(port) + ",u,t);";
	   '#endif'
	   "  return 0;";
           "}"];
