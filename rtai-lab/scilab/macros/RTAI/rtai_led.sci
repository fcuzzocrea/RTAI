function [x,y,typ]=rtai_led(job,arg1,arg2)
//
...
case 'set' then
  ...
  while %t do
    [ok,ip,name,lab]=..
        getvalue('Set RTAI-led block parameters',..
        ['input ports';
	'Led name'],..
...
function [ok,tt]=getCode_led(funam)
   textmp=[
          '#ifndef MODEL'
          '#include <math.h>';
          '#include <stdlib.h>';
  ...
  textmp($+1)='static void * led;'
  textmp($+1)='int i;'
  textmp($+1)='double u[' + string(nin) + '];'
  textmp($+1)='double t = get_scicos_time();' 
  textmp($+1)='  switch(flag) {'
  textmp($+1)='  case 4:'
  textmp($+1)='    led=out_rtai_led_init('+ string(nin)+','+ '""' + name + '"");'
  textmp($+1)='    break;'; 
  textmp($+1)='  case 2:'
  textmp($+1)='    for (i=0;i<' + string(nin) + ';i++) u[i]=block->inptr[i][0];'
  textmp($+1)='    out_rtai_led_output(led,u,t);'
  textmp($+1)='    break;'
  textmp($+1)='  case 5:'
  textmp($+1)='    out_rtai_led_end(led);'
  textmp($+1)='    break;'
  textmp($+1)='  }'
  textmp($+1)='#endif'
  textmp($+1)='}'
