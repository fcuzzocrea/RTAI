...
case 'set' then
  ...
  while %t do
    ...
    label(1)=lab
    funam='o_mbx_ovrwr_send_' + string(port);
    xx=[];ng=[];z=0;
    nx=0;nz=0;
    o=[];
    i=[];
...
case 'define' then
  ...
  gr_i=['xstringb(orig(1),orig(2),[''Mbx Send'';''Overwrite''],sz(1),sz(2),''fill'');']
...
if tt==[] then
  ...
  textmp($+1)='   break;'; 
    l1 = '  out_mbx_ovrwr_send_init(' + string(port) + ',' + string(nin) + ',';
    l2 = '""' + name + '"",""' + ipaddr + '""';
  ...
  ttext=[ttext;'int '+funam+"_bloc_outputs(scicos_block *block,int flag)";
	  "{";
	   ...
	   '#ifdef MODEL'
           "  out_mbx_ovrwr_send_output(" + string(port) + ",u,t);";
	   '#endif'
	   "  return 0;";
           "}"];
  ...
  textmp($+1)='  case 5: '
      ...
      textmp($+1)='   break;'; 
	   ...
	   "{";
	   '#ifdef MODEL'
	   "  out_mbx_ovrwr_send_end(" + string(port) + ");";
	   '#endif'
	   "  return 0;";
           "}"];
  textmp($+1)='  }'
