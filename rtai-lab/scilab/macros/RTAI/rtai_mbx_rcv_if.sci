...
case 'set' then
  ...
  while %t do
    ...
    label(1)=lab
    funam='i_mbx_rcvif_' + string(port);
    xx=[];ng=[];z=0;
    nx=0;nz=0;
    o=[];
    i=[];
...
case 'define' then
  ...
  gr_i=['xstringb(orig(1),orig(2),[''Mbx Rcv'';''no blocking''],sz(1),sz(2),''fill'');']
...
if tt==[] then
  ...
  textmp($+1)='   break;'; 
    l1 = '  inp_mbx_receive_if_init(' + string(port) + ',' + string(nout) + ',';
    l2 = '""' + name + '"",""' + ipaddr + '""';
  ...
  textmp($+1)=' '
	 ...
	 "int i;";
         ...
         '#ifdef MODEL'
         "  inp_mbx_receive_if_input(" + string(port) + ",y,t);";
         '#endif'
	 "  for (i=0;i<" + string(nout) + ";i++) block->outptr[i][0] = y[i];";
 	 ...
 	 textmp($+1)='  case 5: '
         ...
         "{";
         '#ifdef MODEL'
         "  inp_mbx_receive_if_end(" + string(port) + ");";
         '#endif'
         "  return 0;";
         "}"];
  	 textmp($+1)='  }'
