...
case 'set' then
  ...
  while %t do
    ...
    [ok,tt]=getCode_mbx_send_if(funam)
    if ~ok then break,end
...
function [ok,tt]=getCode(_mbx_send_iffunam)
   textmp=[
          '#ifndef MODEL'
          '#include <math.h>';
          '#include <stdlib.h>';
  ...
  textmp($+1)='  case 4:'
  textmp($+1)='    port=out_mbx_send_if_init('+ string(nin)+','+ '""' + name + '"",""'+ipaddr+'"");'
  textmp($+1)='    break;'; 
  textmp($+1)='  case 2:'
  textmp($+1)='    for (i=0;i<' + string(nin) + ';i++) u[i]=block->inptr[i][0];'
  textmp($+1)='    out_mbx_send_if_output(port,u,t);'
  textmp($+1)='    break;'
  textmp($+1)='  case 5:'
  textmp($+1)='    out_mbx_send_if_end(port);'
  textmp($+1)='    break;'
  textmp($+1)='  }'
  textmp($+1)='#endif'
  textmp($+1)='}'
