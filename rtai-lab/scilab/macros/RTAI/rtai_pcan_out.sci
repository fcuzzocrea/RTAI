function [x,y,typ]=rtai_pcan_out(job,arg1,arg2)
//
...
case 'define' then
  ...
  gr_i=['xstringb(orig(1),orig(2),''PCAN OUT'',sz(1),sz(2),''fill'');']
...
if tt==[] then
  ...
  textmp($+1)='{'
  	 ...
  	 textmp($+1)='   break;';
    	 l1 = '  out_pcan_init(' + string(port) + ',0,';
  ...
  ttext=[ttext;'int '+funam+"_bloc_outputs(scicos_block *block,int flag)";
	  "{";
	   ...
	   "  u[0]=block->inptr[0][0];";
	   '#ifdef MODEL'
           "  out_pcan_output(" + string(port) + ",u,t);";
	   '#endif'
	   "  return 0;";
           "}"];
