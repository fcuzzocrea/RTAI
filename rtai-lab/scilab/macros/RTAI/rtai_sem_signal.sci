...
case 'set' then
  ...
  while %t do
    ...
    [ok,tt]=getCode_sem_signal(funam)
    if ~ok then break,end
...
function [ok,tt]=getCode_sem_signal(funam)
   textmp=[
          '#ifndef MODEL'
          '#include <math.h>';
          '#include <stdlib.h>';
  ...
  textmp($+1)='  case 4:'
  textmp($+1)='    port=out_rtai_sem_init(""' + name + '"",""'+ipaddr+'"");'
  textmp($+1)='    break;'; 
  textmp($+1)='  case 2:'
  textmp($+1)='    u[0]=block->inptr[0][0];'
  textmp($+1)='    out_rtai_sem_output(port,u,t);'
  textmp($+1)='    break;'
  textmp($+1)='  case 5:'
  textmp($+1)='    out_rtai_sem_end(port);'
  textmp($+1)='    break;'
  textmp($+1)='  }'
  textmp($+1)='#endif'
  textmp($+1)='}'
