...
case 'set' then
  ...
  oldlb=label(1)
  while %t do
    [ok,port,A,prd,pulse,bias,delay,lab]=..
...
case 'define' then
  port=1;A=1;prd=4;pulse=2;bias=0;delay=0;
  rt_par=[A,prd,pulse,bias,delay];
  rpar=rt_par(:);
  model=scicos_model()
  model.sim=list(' ',2004)
  ...
  label=list([sci2exp(port), sci2exp(A), sci2exp(prd), sci2exp(pulse), sci2exp(bias), sci2exp(delay)],[])

  gr_i=['xstringb(orig(1),orig(2),''RTAI square'',sz(1),sz(2),''fill'');']
