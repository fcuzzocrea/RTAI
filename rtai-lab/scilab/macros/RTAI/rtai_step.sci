function [x,y,typ]=rtai_step(job,arg1,arg2)
//
...
case 'define' then
  ...
  gr_i=['xstringb(orig(1),orig(2),''RTAI step'',sz(1),sz(2),''fill'');']
...
if tt==[] then
  ...
  textmp($+1)='   break;';
  textmp($+1)='  case 1:'
  textmp($+1)='   if (t<block->rpar[1]) block->outptr[0][0]=0.0;'
  textmp($+1)='   else                  block->outptr[0][0]=block->rpar[0];'          
  textmp($+1)='   break;';
  textmp($+1)='  case 5: '
  textmp($+1)='   block->outptr[0][0]=0.0;'
  textmp($+1)='   break;';
