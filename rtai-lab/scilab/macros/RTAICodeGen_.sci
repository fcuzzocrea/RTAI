...
function Makename=gen_make_unix(name,files,libs,Makename)
  ...
  T=["# generated by builder.sce: Please do not edit this file"
     ...
     ascii(9)+"cp $< ."
     ""
     name+"_standalone: $(OBJSSTAN) $(ULIBRARY)"
...
// Modified for RTAI by Roberto Bucher (roberto.bucher@die.supsi.ch)
  ...
  RCode=[];nbrpa=0;strRCode='';lenRCode=[];ntot_r=0;
  if size(rpar,1) <> 0 then

    RCode=[RCode;'static double RPAR1[ ] = {'];		
    par_len = length(rpptr)-1;    
    for i=1:par_len 
	...
	if and(aaa+bbb~= ['INPUTPORTEVTS'; ...
	  ...
	  if stripblanks(OO.graphics.exprs)~=emptystr() then
	    RCode=[RCode;cformatline('Exprs: '+strcat(OO.graphics.exprs,","),70)];
	  end
  ...
  if size(ipar,1) <> 0 then
	...
	if and(aaa+bbb~= ['INPUTPORTEVTS';'OUTPUTPORTEVTS';'EVTGEN_f']) then
	  ...
	  if stripblanks(OO.graphics.exprs)~=emptystr() then
	    ICode=[ICode;cformatline('Exprs: '+strcat(OO.graphics.exprs,","),70)];
	  end
	  if stripblanks(OO.graphics.id)~=emptystr() then
	    str_id = string(OO.graphics.id);
          else
...
//Author : Roberto Bucher (roberto.bucher@die.supsi.ch)
  ...
  if n~=0 then  
    ...
    unix_w(exec_str);
    exec_str = 'make -f '+ rdnom + '_Makefile ' + rdnom + '_standalone';
    unix_w(exec_str)
  end
