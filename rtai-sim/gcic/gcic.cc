...
static struct tool_options {
    ...
    { "-xenomai", &FlagXenomai, 1, NULL },
    { "-vrtai", &FlagVrtai, 1, NULL },
    { NULL, NULL, 0, NULL }
};
...

{
    ...
    fprintf(stderr,"            [--xenomai]\n");
    fprintf(stderr,"            [--vrtai]\n");
				...
				int cplus)
{
    ...
    cmdLine += " -isystem ";
    cmdLine += BackendPrefix;
    cmdLine += CONFIG_RTAI_MVM_BUILD_STRING;
    cmdLine += "/include";
...

{
	...
	argStage0.append(new LString("-D__MVM__"));

	if (FlagKTag)
	    argStage0.append(new LString("-D__MVM__KERNEL_SCOPE__"));

	if (FlagITag)
	    argStage0.append(new LString("-D__MVM__SKIN_SCOPE__"));

	if (FlagATag)
	    argStage0.append(new LString("-D__MVM__USER_SCOPE__"));
	...
	if (FlagVrtai)
	    // Add virtual RTAI dir to allow direct inclusion of
	    // rtai.h and asm/rtai.h.
	    argStage0.append(new LString(incDir + "/sim/vrtai"));
	...
	if (!FlagNoMvmLib)
	    ...
	    if (FlagXenomai || !FlagVrtai)
		CCCmdString += " -lxenomai_s";
	    else if (FlagVrtai)
		CCCmdString += " -lrtai_s";
	    }
